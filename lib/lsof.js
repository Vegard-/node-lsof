...
var raw = exports.raw = function(pid, fn) {
    ...
    cp.exec('lsof +c 0 -p ' + pid, function(err, d) {
        d = d.split('\n');
        var data = [];
...
var rawTcpPort = exports.rawTcpPort = function(host, port, fn) {
    cp.exec('lsof +c 0 -nPi TCP@' + host + ':' + port, function(err, d) {
        d = d.split('\n');
        var data = [];
        var headers = d[0].toLowerCase().split(/\s+/);
        headers.forEach(function(v, k) {
...
var rawUdpPort = exports.rawUdpPort = function(host, port, fn) {
    cp.exec('lsof +c 0 -nPi UDP@' + host + ':' + port, function(err, d) {
        d = d.split('\n');
        var data = [];
        var headers = d[0].toLowerCase().split(/\s+/);
        headers.forEach(function(v, k) {
