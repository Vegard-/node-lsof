 ...
 */
var rawFilter = exports.rawFilter = function(options, fn) {
    let cmd = [
      'lsof '
    ];
    if (options.uid){
    ...
    cmd.push('+c 0 -nPi ');
    if (options.protocol){
      cmd.push(options.protocol);
    }
    let hasValidPort = typeof options.port != 'undefined';
    if (hasValidPort || options.host){
      if (!options.host){
        options.host = '0.0.0.0';
      }
      cmd.push('@' + options.host);
      if (hasValidPort){
        cmd.push(':' + options.port);
      }
    }
    cp.exec(cmd.join(''), function(err, d) {
        d = d.split('\n');
        var data = [];
        var headers = d[0].toLowerCase().split(/\s+/);
        headers.forEach(function(v, k) {
